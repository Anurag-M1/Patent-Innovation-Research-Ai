...
def display_menu():
    ...
    print("\n" + "=" * 60)
    print("  PATENT RESEARCH ASSISTANT  ")
    print("=" * 60)
...
def run_complete_analysis():
    ...
    default_model = os.getenv("OPENROUTER_MODEL", "qwen/qwen3-coder")
    model_name = input(
        f"Enter the OpenRouter model to use (default: {default_model}): "
    )
    if not model_name:
...
def check_system_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Models visible: {len(models)}"
            )
        else:
            print(
                f"❌ OpenRouter connection: Failed (Status code: {response.status_code})"
            )
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\nExiting Patent Research Assistant. Goodbye!")
            break
        else:
